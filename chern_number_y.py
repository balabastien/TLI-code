...
for i in range(Nx):
    for j in range(int(np.floor(Ny/2))-1):
        for k in range(Nz):
            for m in range(2):
...
for i in range(Nx):
    for j in range(int(np.floor(Ny/2))+1,Ny):
        for k in range(Nz):
            for m in range(2):
...
leftsystem = []
for i in range(Nx):
    for j in range(int(np.floor(Ny/2))):
        for k in range(Nz):
            for m in range(2):
                leftsystem.append(arraytoindex(i,j,k,m))

def submatrixoperatorz(l):
    return matrixoperatorz(l)[leftsystem,:][:,leftsystem]
...
for t in tspace:
   ...
   commutz = np.zeros((len(leftsystem),len(leftsystem)),dtype=np.complex128)
   superproj=[]
   chernprodan=0
   ...
   for l in range(1,Nz):
       commutz += clambda(l,Nz)* ( submatrixoperatorz(l) @ subprojector @ submatrixoperatorz(-l))
       commutx += clambda(l,Nx)* ( submatrixoperatorx(l) @ subprojector @ submatrixoperatorx(-l)) 
   commutator = np.dot(commutx,commutz)-np.dot(commutz,commutx)
   superproj = np.dot(subprojector,commutator)
   chernprodan = -2*np.pi*1j/(Nx*Nz) * np.trace(superproj)
   print(chernprodan)
...
a_file = open("chern151515facettey.txt", "w")
